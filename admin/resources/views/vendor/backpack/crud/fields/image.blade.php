    ...
    <div class="list" data-field-name="{{ $field['name'] }}">
        ...
        @else
            <div class="input-group input-group-sm item">
                <div class="item-img" @if($value) style='background-image: url("{{ Storage::disk("elfinder")->url($value) }}");' @endif></div>
                ...
                if ($input.val() != '' && $input.val() != null && $multiple === 'true') {
                    ...
                    if (Array.isArray($paths) && $paths.length) {
                        ...
                        $paths.forEach(function (path) {
                            ...
                            newInput.find('.item-img').attr('style', `background-image: url("{{ Storage::disk('elfinder')->url('') }}${path}")`);
                            $list.append(newInput);
                        });
                    }
                } else if ($multiple !== 'true' && $input.val()) {
                    $list.find('.item-img').attr('style', `background-image: url("{{ Storage::disk('elfinder')->url('') }}${$input.val()}")`)
                }
                ...
                if ($multiple === 'true') {
                    ...
                    element.on('createInputsForItemsSelectedWithElfinder', element, function(event, files) {
                        files.forEach(function (file) {
                            ...
                            newInput.find('.item-img').attr('style', `background-image: url("{{ Storage::disk('elfinder')->url('') }}${url}")`);
                            $list.append(newInput);
                        });
                    ...
                    element.on('createInputsForItemsSelectedWithElfinder', element, function(event, files) {
                        ...
                        $list.find('.item-img').attr('style', `background-image: url("{{ Storage::disk('elfinder')->url('') }}${url}")`);
                        $input.val(url);
                    });
                }
            }
